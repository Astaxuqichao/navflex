// New RecoveryRogMapServer implementation using NavflexActionBase framework.
// Original version preserved in behavior_server_bak.cpp

#include "navflex_nav/rogmap_nav/behavior_action_rogmap_server.hpp"
...
: nav2_util::LifecycleNode(
  ...
  default_ids_{},
  default_types_{},
  global_rog_map_ros_(global_rog_map_ros),
  local_rog_map_ros_(local_rog_map_ros),
...
nav2_util::CallbackReturn RecoveryRogMapServer::on_configure(
  const rclcpp_lifecycle::State & /*state*/)
{
  RCLCPP_INFO(get_logger(), "Configuring RecoveryRogMapServer");
  ...
  action_server_ = rclcpp_action::create_server<ActionDummyBehavior>(
    ...
    std::bind(&RecoveryRogMapServer::handleGoalDummyBehavior, this, _1, _2),
    std::bind(&RecoveryRogMapServer::cancelActionDummyBehavior, this, _1),
    std::bind(&RecoveryRogMapServer::callActionDummyBehavior, this, _1),
...
bool RecoveryRogMapServer::loadBehaviorPlugins()
{
  auto node = shared_from_this();
  for (size_t i = 0; i < behavior_ids_.size(); i++) {
    ...
    try {
      ...
      navflex_rogmap_core::Recovery::Ptr behavior =
        plugin_loader_.createUniqueInstance(behavior_types_[i]);
      behavior->configure(
        node, behavior_ids_[i], local_rog_map_ros_->getTfBuffer(),
        local_rog_map_ros_->getRogMap());
  ...
  RCLCPP_INFO(
    get_logger(), "Behavior Server has %s behaviors available.",
    behavior_ids_concat_.c_str());
  return true;
}

nav2_util::CallbackReturn RecoveryRogMapServer::on_activate(
  const rclcpp_lifecycle::State & /*state*/)
{
  RCLCPP_INFO(get_logger(), "Activating RecoveryRogMapServer");
...
nav2_util::CallbackReturn RecoveryRogMapServer::on_deactivate(
  const rclcpp_lifecycle::State & /*state*/)
{
  RCLCPP_INFO(get_logger(), "Deactivating RecoveryRogMapServer");
  if (behavior_action_) {
    behavior_action_->cancelAll();
  }
  for (auto & [id, beh] : behaviors_) {
...
nav2_util::CallbackReturn RecoveryRogMapServer::on_cleanup(
  const rclcpp_lifecycle::State & /*state*/)
{
  RCLCPP_INFO(get_logger(), "Cleaning up RecoveryRogMapServer");
  ...
  behaviors_.clear();
  global_rog_map_ros_.reset();
  local_rog_map_ros_.reset();
  return nav2_util::CallbackReturn::SUCCESS;
}

nav2_util::CallbackReturn RecoveryRogMapServer::on_shutdown(
  const rclcpp_lifecycle::State & /*state*/)
{
  RCLCPP_INFO(get_logger(), "Shutting down RecoveryRogMapServer");
  return nav2_util::CallbackReturn::SUCCESS;
}

rclcpp_action::GoalResponse RecoveryRogMapServer::handleGoalDummyBehavior(
...
rclcpp_action::CancelResponse RecoveryRogMapServer::cancelActionDummyBehavior(
...
void RecoveryRogMapServer::callActionDummyBehavior(
  ServerGoalHandleDummyBehaviorPtr goal_handle)
{
  ...
  if (behaviors_.find(behavior_name) == behaviors_.end()) {
    ...
    result->message = "Behavior '" + behavior_name + "' not found. Available: " +
      behavior_ids_concat_;
  ...
  RCLCPP_INFO(
    get_logger(), "[BehaviorServer] calling behavior plugin id=%s",
    behavior_name.c_str());
...
BehaviorExecution::Ptr RecoveryRogMapServer::newBehaviorExecution(
  const std::string & behavior_name)
{
  auto node = shared_from_this();
  return std::make_shared<BehaviorExecution>(
    behavior_name,
    std::make_shared<RogMapRecoveryAdapter>(behaviors_.at(behavior_name)),
