// New ControllerRogMapServer implementation using NavflexActionBase framework.
// Original version preserved in controller_server_bak.cpp

#include "navflex_nav/rogmap_nav/controller_action_rogmap_server.hpp"
...
: nav2_util::LifecycleNode(
  ...
  default_ids_{},
  default_types_{},
  rog_map_ros_(rog_map_ros),
  robot_info_(robot_info),
  name_action_follow_path_("follow_path")
{
  RCLCPP_INFO(
    get_logger(), "[ControllerServer] created action_server=%s",
...
nav2_util::CallbackReturn ControllerRogMapServer::on_configure(
  const rclcpp_lifecycle::State & /*state*/)
{
  ...
  for (size_t i = 0; i < controller_ids_.size(); i++) {
    try {
      ...
      navflex_rogmap_core::Controller::Ptr ctrl =
        lp_loader_.createUniqueInstance(controller_types_[i]);
      RCLCPP_INFO(
        get_logger(), "[ControllerServer] loaded controller id=%s type=%s",
        controller_ids_[i].c_str(), controller_types_[i].c_str());
      ctrl->configure(
        node, controller_ids_[i],
        rog_map_ros_->getTfBuffer(), rog_map_ros_->getRogMap());
  ...
  RCLCPP_INFO(
    get_logger(), "[ControllerServer] controllers=[%s]",
  ...
  speed_limit_sub_ = create_subscription<nav2_msgs::msg::SpeedLimit>(
    ...
    std::bind(&ControllerRogMapServer::speedLimitCallback, this, _1));
  ...
  action_server_follow_path_ =
    ...
    std::bind(&ControllerRogMapServer::handleGoalFollowPath, this, _1, _2),
    std::bind(&ControllerRogMapServer::cancelActionFollowPath, this, _1),
    std::bind(&ControllerRogMapServer::callActionFollowPath, this, _1),
...
nav2_util::CallbackReturn ControllerRogMapServer::on_activate(
  const rclcpp_lifecycle::State & /*state*/)
{
  RCLCPP_INFO(
    get_logger(), "[ControllerServer] activating controllers=[%s]",
    controller_ids_concat_.c_str());
  ...
  dyn_params_handler_ = node->add_on_set_parameters_callback(
    std::bind(&ControllerRogMapServer::dynamicParametersCallback, this, _1));

  return nav2_util::CallbackReturn::SUCCESS;
}

nav2_util::CallbackReturn ControllerRogMapServer::on_deactivate(
  const rclcpp_lifecycle::State & /*state*/)
{
  RCLCPP_INFO(get_logger(), "Deactivating ControllerRogMapServer");
  if (controller_action_) {
    controller_action_->cancelAll();
  }
  for (auto & [id, ctrl] : controllers_) {
...
nav2_util::CallbackReturn ControllerRogMapServer::on_cleanup(
  const rclcpp_lifecycle::State & /*state*/)
{
  RCLCPP_INFO(get_logger(), "Cleaning up ControllerRogMapServer");
...
nav2_util::CallbackReturn ControllerRogMapServer::on_shutdown(
  const rclcpp_lifecycle::State & /*state*/)
{
  RCLCPP_INFO(get_logger(), "Shutting down ControllerRogMapServer");
  return nav2_util::CallbackReturn::SUCCESS;
}

rclcpp_action::GoalResponse ControllerRogMapServer::handleGoalFollowPath(
...
rclcpp_action::CancelResponse ControllerRogMapServer::cancelActionFollowPath(
...
void ControllerRogMapServer::callActionFollowPath(
  ServerGoalHandleFollowPathPtr goal_handle)
{
  ...
  if (!findControllerId(goal->controller_id, controller_id)) {
    ...
    RCLCPP_ERROR(
      get_logger(), "Requested controller '%s' not found.",
      goal->controller_id.c_str());
...
bool ControllerRogMapServer::findControllerId(
...
ControllerExecution::Ptr ControllerRogMapServer::newControllerExecution(
  ...
  double yaw_goal_tolerance)
{
  ...
  return std::make_shared<ControllerExecution>(
    controller_id,
    std::make_shared<RogMapControllerAdapter>(controllers_.at(controller_id)),
    goal_checker,
    xy_goal_tolerance,
    yaw_goal_tolerance,
    robot_info_,
...
void ControllerRogMapServer::speedLimitCallback(
...
void ControllerRogMapServer::publishZeroVelocity()
{
...
rcl_interfaces::msg::SetParametersResult ControllerRogMapServer::dynamicParametersCallback(
