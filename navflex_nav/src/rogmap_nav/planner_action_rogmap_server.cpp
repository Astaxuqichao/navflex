// Implementation of PlannerRogMapServer
...
#include "navflex_nav/rogmap_nav/planner_action_rogmap_server.hpp"
 ...
 * 1. Get costmap from rog_map_ros and extract costmap dimensions
 * 2. Retrieve TF buffer from rog_map_ros for coordinate transformations
 ...
 *
 * **Plugin System:**
 * Uses pluginlib to dynamically load navflex_rogmap_core::GlobalPlanner implementations:
 * - NavfnPlanner (default)
 * - SmacPlanner
 * - ThetaStarPlanner
 * - Other custom planners implementing navflex_rogmap_core::GlobalPlanner interface
 *
 * **Error Handling:**
 ...
 * - Costmap and TF resources come from shared rog_map_ros object
 * - All plugins are configured with same TF buffer and costmap
 * - Action server callbacks are bound to PlannerRogMapServer methods
 */
nav2_util::CallbackReturn PlannerRogMapServer::on_configure(
  const rclcpp_lifecycle::State & /*state*/)
{
  RCLCPP_INFO(get_logger(), "[PlannerServer] configuring");

  tf_ = rog_map_ros_->getTfBuffer();
  ...
  for (size_t i = 0; i != planner_ids_.size(); i++) {
    try {
      ...
      navflex_rogmap_core::GlobalPlanner::Ptr planner =
        gp_loader_.createUniqueInstance(planner_types_[i]);
      RCLCPP_INFO(
        get_logger(), "[PlannerServer] loaded planner id=%s type=%s",
        planner_ids_[i].c_str(), planner_types_[i].c_str());
      planner->configure(node, planner_ids_[i], tf_, rog_map_ros_->getRogMap());
  ...
  action_server_get_path_ptr_ = rclcpp_action::create_server<ActionToPose>(
    ...
    std::bind(
      &navflex_nav::PlannerRogMapServer::handleGoalGetPath, this,
      _1, _2),
    std::bind(
      &navflex_nav::PlannerRogMapServer::cancelActionGetPath, this,
      _1),
    std::bind(
      &navflex_nav::PlannerRogMapServer::callActionGetPath, this,
      _1),
 ...
 */
nav2_util::CallbackReturn PlannerRogMapServer::on_activate(
  const rclcpp_lifecycle::State & /*state*/)
{
  RCLCPP_INFO(
  ...
  //     std::bind(&PlannerRogMapServer::dynamicParametersCallback, this, _1));
 ...
 */
nav2_util::CallbackReturn PlannerRogMapServer::on_deactivate(
  const rclcpp_lifecycle::State & /*state*/)
{
 ...
 * - Costmap cleanup handled by rog_map_ros
 * - Reset order ensures no lingering references between components
 */
nav2_util::CallbackReturn PlannerRogMapServer::on_cleanup(
  const rclcpp_lifecycle::State & /*state*/)
{
 ...
 */
nav2_util::CallbackReturn PlannerRogMapServer::on_shutdown(
 ...
 */
rclcpp_action::GoalResponse PlannerRogMapServer::handleGoalGetPath(
 ...
 */
void PlannerRogMapServer::callActionGetPath(ServerGoalHandleGetPathPtr goal_handle)
{
  ...
  navflex_rogmap_core::GlobalPlanner::Ptr planner_ptr = nullptr;
  ...
  navflex_nav::PlannerExecution::Ptr planner_execution =
...
rclcpp_action::CancelResponse PlannerRogMapServer::cancelActionGetPath(
...
navflex_nav::PlannerExecution::Ptr
PlannerRogMapServer::newPlannerExecution(
  const std::string & plugin_name,
  const navflex_rogmap_core::GlobalPlanner::Ptr & plugin_ptr)
{
  auto node = shared_from_this();
  return std::make_shared<navflex_nav::PlannerExecution>(
    plugin_name, std::make_shared<RogMapPlannerAdapter>(plugin_ptr), robot_info_, node);
}
