...
#include "navflex_nav/costmap_nav/planner_action_costmap_server.hpp"
  ...
  const rclcpp_lifecycle::State & /*state*/)
{
  ...
  action_server_get_path_ptr_ = rclcpp_action::create_server<ActionToPose>(
    ...
    std::bind(
      &navflex_nav::PlannerCostmapServer::handleGoalGetPath, this,
      _1, _2),
    std::bind(
      &navflex_nav::PlannerCostmapServer::cancelActionGetPath, this,
      _1),
    std::bind(
      &navflex_nav::PlannerCostmapServer::callActionGetPath, this,
      _1),
  ...
  const nav2_core::GlobalPlanner::Ptr & plugin_ptr)
{
  ...
  return std::make_shared<navflex_nav::PlannerExecution>(
    plugin_name, std::make_shared<Nav2PlannerAdapter>(plugin_ptr), robot_info_, node);
}
