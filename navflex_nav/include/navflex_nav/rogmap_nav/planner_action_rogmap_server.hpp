 ...
 * → cleanup)
 */
class PlannerRogMapServer : public nav2_util::LifecycleNode
{
public:
  // ========== Type Definitions ==========
  /// Map of planner plugins indexed by name
  using PlannerMap =
    std::unordered_map<std::string, navflex_rogmap_core::GlobalPlanner::Ptr>;
  ...
  /**
   * @brief Constructor for PlannerRogMapServer
   *
   ...
   *
   * @param rog_map_ros Shared pointer to the global costmap
  ...
  /**
   * @brief Destructor for PlannerRogMapServer
   */
  ~PlannerRogMapServer();
...
protected:
  ...
  navflex_nav::PlannerExecution::Ptr newPlannerExecution(
...
private:
  ...
  pluginlib::ClassLoader<navflex_rogmap_core::GlobalPlanner> gp_loader_;
  ...
  std::shared_ptr<navflex_rog_map::RogMapROS> rog_map_ros_;
